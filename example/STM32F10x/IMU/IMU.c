...
static void IMU_Init_Timer(void) {
	...
	timer_initstruct.TIM_Period = 49999;
...
static void IMU_Get_Raw_Data(IMU_Data_t *imu_data) {
	MPU6050_Get_Gyro(&imu_data->gyro_x, &imu_data->gyro_y, &imu_data->gyro_z);
	MPU6050_Get_Accel(&imu_data->accel_x, &imu_data->accel_y, &imu_data->accel_z);
